const cols = [
    ...
    {
        title: "Support",
        links: ["Help Center", "Community", "Status", "Privacy Policy", "Terms of Service"],
    },
    {
        title: "Legal",
        links: ["Privacy", "Terms", "HIPAA Compliance", "GDPR", "Cookie Policy"],
    },
];
        ...
        <footer className="border-t border-white/[0.06] py-16 px-6">
            <div className="max-w-7xl mx-auto">
                <div className="grid grid-cols-2 md:grid-cols-6 gap-10 mb-12">
                    ...
                    <div className="col-span-2">
                        ...
                        <p className="text-slate-500 text-sm leading-relaxed max-w-xs">
                            AI-powered cephalometric analysis trusted by orthodontists worldwide. Precision in seconds.
                        </p>
                        <div className="flex gap-4 mt-6">
                            {socials.map((s) => (
                                <a key={s.label} href={s.href} aria-label={s.label} className="text-slate-500 hover:text-white transition-colors">
                                    {s.icon}
                                </a>
                            ))}
                        </div>
                    ...
                    {cols.map((col) => (
                        <div key={col.title}>
                            <p className="text-white text-xs font-semibold tracking-wider uppercase mb-4">{col.title}</p>
                ...
                <div className="border-t border-white/[0.06] pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-slate-600 text-xs">
                    <p>© {new Date().getFullYear()} CephaloAI. All rights reserved.</p>
                    <p>Built with ❤️ for orthodontists worldwide.</p>
                </div>
            </div>
        </footer>
    );
